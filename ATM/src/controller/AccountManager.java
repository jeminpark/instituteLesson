...
public class AccountManager {
	...
	public void createAcc() {
		//현재 로그인 중인 회원이 보유한 계좌의 수가 확인이되고
		//그계좌의 수가 Account.MAX값을 기준으로 처리가 두가지로 나뉜다.
...
//		}
		...
		if(cnt < 3) {
			...
			System.out.println("최대 계좌 개수를 초과했습니다.");
		}
				
	}
	public void deleteAcc() {
		printAcc();
		System.out.print("삭제할 계좌선택: ");
	...
	private int findIndex(String input) { //로그인한 사용자에 대해서
		int index = -1;
		try {
			int userCode = getUser(Bank.log).getUserCode();
			int cnt = getUser(Bank.log).getAccCnt();
